...
namespace Common
{
    public struct IniValue
    {
        ...
        public float ToSingle(float valueIfInvalid = 0)
        {
            if (TryConvertSingle(out float res))
            {
                return res;
            }
            return valueIfInvalid;
        ...
        public bool TryConvertSingle(out float result)
        {
            if (Value == null)
            {
            ...
            if (TryParseSingle(Value.Trim(), out result))
            {
                return true;
            }
            return false;
    ...
    public class IniFile : IEnumerable<KeyValuePair<string, IniSection>>, IDictionary<string, IniSection>, IDisposable
    {
        ...
        {
            string value = this[section][key].Value;

            if (value == null)
            {
                value = defaultValue;
                SaveValue(LoadPath, section, key, defaultValue);
            }
        ...
        public int GetValue(string section, string key, int defaultValue)
        {
            ...
            if (value == null)
            {
                ...
                SaveValue(LoadPath, section, key, value);
            }
        ...
        {
            bool value;
            string s = this[section][key].Value;
            if (s != null)
            {
            ...
            {
                value = defaultValue;
                string strValue = value ? bool.TrueString : bool.FalseString;
                SaveValue(LoadPath, section, key, strValue);
            }
            return value;
        }

        public bool SetValue(string section, string key, object value)
        {
            string strValue = "";
            if (value != null)
            {
            ...
            SaveValue(LoadPath, section, key, strValue);
            return strValue == this[section][key].Value;
        }
        ...
        public void SaveValue(string path, string section, string key, string value, FileMode mode = FileMode.Create)
        {
